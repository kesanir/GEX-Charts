...
# Tickers that need special handling for spot price lookups.
#
# YF_ALIASES   : yfinance symbol map  (SPX → ^GSPC, VIX → ^VIX, etc.)
# POLY_AGG_PREFIX : Polygon aggs ticker prefix for indices  (SPX → I:SPX)
#
# Add entries here to support any non-equity index.
YF_ALIASES: Dict[str, str] = {
    "SPX":  "^GSPC",
    "NDX":  "^NDX",
    "RUT":  "^RUT",
    "VIX":  "^VIX",
...
def _poly_ticker(ticker: str) -> str:
    """Return the Polygon aggs ticker symbol for any underlying (index or equity)."""
    return POLY_AGG_PREFIX.get(ticker.upper(), ticker.upper())


def _yf_ticker(ticker: str) -> str:
    """Return the yfinance symbol for any underlying."""
    return YF_ALIASES.get(ticker.upper(), ticker.upper())
...
def _agg_close(ticker: str, lookback_ms: int, label: str) -> float:
    ...
    poly_t  = _poly_ticker(ticker)   # e.g. SPX → I:SPX
    d = _get(
        f"/v2/aggs/ticker/{poly_t}/range/1/minute/{ago_ms}/{now_ms}",
...
def _prev_close(ticker: str) -> float:
    ...
    poly_t = _poly_ticker(ticker)
    d = _get(f"/v2/aggs/ticker/{poly_t}/prev", {"adjusted": "true"})
    if not d:
        return 0.0
    try:
        return float(((d.get("results") or [{}])[0]).get("c") or 0)
...
def get_spot(ticker: str) -> float:
    ...
    if _is_market_open():
        ...
        poly_t = _poly_ticker(ticker)   # e.g. SPX → I:SPX
        d = _get(
            f"/v2/aggs/ticker/{poly_t}/range/1/day/{ago_ms}/{now_ms}",
        ...
        if d:
            try:
                ...
                bar_date = _et_now().date() if bar_ts_s == 0 else                     (datetime.utcfromtimestamp(bar_ts_s) + __import__('datetime').timedelta(hours=-5)).date()
        ...
        p = _agg_close(ticker, lookback_ms=7 * 24 * 3600 * 1000, label="aggs_1min_recent")
        if p > 0:
            return p

        # Final fallback — yfinance works for all tickers including indices (^GSPC etc.)
        # Essential for SPX/NDX/VIX which have no Polygon equity data on Options Starter.
...
def get_spot_yfinance(ticker: str) -> float:
    ...
    """
    yf_sym = _yf_ticker(ticker)   # SPX → ^GSPC, else unchanged
    try:
        import yfinance as yf
        t = yf.Ticker(yf_sym)
        p = getattr(t.fast_info, "last_price", None)
        if p and p > 0:
            log.info("[spot] %s = %.2f via yfinance (%s)", ticker, float(p), yf_sym)
            return float(p)
        # fallback to .info (slower but more complete)
        p = t.info.get("regularMarketPrice") or t.info.get("currentPrice")
        if p and p > 0:
            log.info("[spot] %s = %.2f via yfinance.info (%s)", ticker, float(p), yf_sym)
            return float(p)
    except ImportError:
        log.debug("[spot] yfinance not installed — pip install yfinance")
    except Exception as e:
        log.debug("[spot] yfinance error for %s (%s): %s", ticker, yf_sym, e)
    return 0.0
...
class StockTradeStream:
    ...
    def run(self) -> None:
        ...
        # Indices have no tape prints — skip WS entirely, rely on yfinance poll
        if self.ticker in YF_ALIASES:
            log.info("[ws-stocks:%s] index ticker — skipping stock WS, yfinance poll active",
                     self.ticker)
            return
        import websocket as _wslib
        self._running = True
