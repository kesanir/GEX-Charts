...
class AlertEngine:
    ...
    def check(self, ticker: str, engine: GEXEngine, levels: dict) -> List[dict]:
        ...
        def _alert(atype, severity, msg, extra=None):
            a = {
...
def _run_bootstrap(ticker: str) -> bool:
    ...
    try:
        ...
        if e.spot_price <= 0 or not e.contract_data:
            log.error("[%s] Bootstrap produced no usable data", ticker)
            return False
    ...
    while True:
        ...
        if spot > 0:
            ...
            if prev_spot_for_cache > 0:
                ...
                if move_pct >= 0.10:
                    try:
                        _update_state_cache(engine, ticker_up)
    ...
    while True:
        ...
        async with lock:
            ...
            if ok and ticker in _engines:
                ...
                new_alerts = _alert_engine.get_alerts(ticker, limit=5)
                if new_alerts:
                    await _broadcast(ticker, {"event": "alerts", "alerts": new_alerts})
...
@app.get("/api/alerts")
async def get_alerts(ticker: Optional[str] = None, limit: int = 50):
    t = ticker.upper() if ticker else None
    return {"alerts": _alert_engine.get_alerts(ticker=t, limit=limit)}
